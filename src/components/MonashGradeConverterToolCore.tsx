...
interface MonashGradeConverterToolCoreProps {
  enableProductPopup?: boolean;
}

export default function MonashGradeConverterToolCore({ enableProductPopup = true }: MonashGradeConverterToolCoreProps) {
  ...
  const userReady =
    fromField === 'mark' ? markInput !== '' : fromField === 'gpa' ? gpaInput !== '' : true;

  const { popupOpen, setPopupOpen, recommendation } = useDelayedProductPopup({
    enabled: enableProductPopup,
    hasResult: result !== null,
    userReady,
    route: '/monash-grade-converter',
    subjects: [{ code: 'CONV', mark: result?.mark ?? null }],
  });
