...
export default function AmazonCtaButton({
  ...
  return (
    <a
      ...
      className={`group flex items-center justify-center gap-1.5 w-full py-2.5 px-2 rounded-xl font-black text-xs transition-all duration-300 shadow-md active:scale-[0.98] cursor-pointer text-center ${
        isHovered
          ? 'bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 text-white shadow-emerald-500/30 scale-[1.02]'
          : 'bg-gradient-to-r from-amber-400 via-amber-300 to-amber-400 text-slate-950 shadow-amber-500/20 animate-pulseScale'
      } ${className}`}
    >
      {isHovered ? (
        <>
        ...
        <>
          ...
          <span className="whitespace-normal leading-tight break-words text-center">{defaultText}</span>
