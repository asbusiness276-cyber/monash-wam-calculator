...
export default function AmazonResultPopUpModal({ hasResult }: AmazonResultPopUpModalProps) {
  ...
  const product = AMAZON_STUDENT_PRODUCTS[5]; // Atomic Habits Box Set
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg overflow-hidden rounded-3xl bg-slate-900 border-2 border-amber-400 p-6 md:p-8 text-white shadow-2xl animate-scale-up">
        ...
        <div className="flex items-center justify-between gap-2 mb-4 pb-3 border-b border-slate-800">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-gradient-to-r from-red-600 to-rose-500 text-white shadow-[0_0_15px_rgba(225,29,72,0.5)] animate-pulse">
            <Flame className="w-4 h-4 fill-white" />
            <span>⚡ URGENT: FLASH SALE (ALMOST GONE)</span>
          </div>

          <div className="flex items-center gap-1 text-xs font-black text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/50 shadow-[0_0_10px_rgba(245,158,11,0.3)]">
          ...
          <div className="sm:col-span-7 space-y-2">
            ...
            <ul className="space-y-1 pt-1">
              {product.keyBenefits.slice(0, 2).map((benefit, i) => (
                <li key={i} className="flex items-center gap-1.5 text-[11px] text-slate-300 font-semibold">
